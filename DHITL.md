...
We identify ten structural failure modes — conditions under which DHITL could fail to maintain meaningful democratic oversight of AI systems. Explicit failure mode analysis is, in our view, a necessary component of any credible safety framework.
...
**S5 (Physical Distribution)**: No single actor controls a sufficient fraction of Layer 1 (energy) or Layer 5 (compute) to unilaterally override Layer 3 decisions.
...
- Actors whose interests diverge from collective welfare control >30% of Layer 1 or Layer 5 resources (S5 violation)
...
- Any single actor accumulates > 20% of global Compute Exergy capacity (S5 violation warning threshold)
...
2. The five-layer infrastructure model as a necessary precondition for democratic HITL, with explicit formal stability conditions.

3. Compute Exergy (Xc) as an economically tractable unit for democratic oversight — a physical-quality-governance-adjusted measure of AI compute that enables market mechanisms to align with democratic accountability requirements.
...
The DHITL framework makes this claim precise. It specifies the five infrastructure layers required for democratic governance of AI systems, the economic unit (Compute Exergy) that makes this governance tractable, the formal stability conditions under which democratic correction remains effective, and the ten structural failure modes that must be explicitly managed.
...
The sun does not belong to anyone. The compute it powers should not either.
...
Mariani, J. H. R. (2026). Infrastructure is all you need: A three-layer architecture for sovereign AI governance. *Companion working paper, developed in parallel*, Institut Mariani R&D. `github.com/JeanHuguesRobert`
...
## Appendix A: Formal Definition of Compute Exergy

Let Ω be the set of all AI compute operations in a given time period T and jurisdiction J.

For any element ω ∈ Ω, the Compute Exergy is:

```
Xc(ω) = E(ω) × η_hw(ω) × η_sys(ω) × η_sla(ω) × η_gov(ω)
```

The total Compute Exergy of jurisdiction J in period T is:

```
XcJ,T = ∫_Ω Xc(ω) dω
```

The **Compute Exergy Concentration Index (CECI)** for period T is the Herfindahl-Hirschman Index applied to the distribution of XcJ,T across actors:

```
CECI_T = Σ_i (XcJ,T,i / XcJ,T)²
```

where XcJ,T,i is the Compute Exergy attributable to actor i.

**Safety threshold**: CECI_T > 0.25 is a warning threshold for Layer 5 concentration (S5 violation warning). CECI_T > 0.40 constitutes a structural instability condition requiring emergency governance response.

These thresholds are analogous to antitrust Herfindahl-Hirschman thresholds used in market concentration analysis, applied to the domain of democratic AI governance.
...
**The anti-reification invariant**: The Cogentigram is not the Cogentia. The map is not the territory. This is not merely a philosophical distinction — it is safety-relevant. An AI system that treats its representation of a person as equivalent to the person will systematically substitute its model of preferences for the person's actual preferences. This constitutes a form of cognitive capture that directly violates the Layer 4 sovereignty condition of DHITL.

**The PrivAI regulatory layer**: The Cogentia framework is governed by a dedicated non-profit (PrivAI) responsible for ethical standards, data minimization, GDPR compliance (explicit exclusion of Article 9 sensitive categories from scoring), and quality certification. This governance structure is itself an instantiation of the Layer 3/Layer 5 separation DHITL requires: the technical system (Cogentiscope/Cogentigram) is regulated by a democratically accountable institution (PrivAI), not self-regulated.

**Operational constraints reproduced from the Cogentia specification**:
...
- All scoring evidence is sanitized server-side (masking of identifying information) before storage.
- Post-analysis user feedback creates an empirical validation loop for the protocol's reliability.

The Cogentia framework's roadmap includes a "Cogentia Match" (inter-profile complementarity scoring) and a "Cogentia Certified" label — developments that, if governed under DHITL-compatible democratic oversight, would constitute a concrete implementation of augmented cognitive sovereignty at scale.

*Reference*: Robert, J. H. N. (2026). Cogentia (KYS) — Personal AI psychometric MVP. GitHub repository: `github.com/JeanHuguesRobert/cogentia`. Conceptual priority claimed. Regulated by PrivAI.
